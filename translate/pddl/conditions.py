...
class Condition:
    ...
    def instantiate_with_delete_effects(self, var_mapping, init_facts, fluent_facts, result):
        raise ValueError("Cannot instantiate condition: not normalized")
...
class Conjunction(JunctorCondition):
    ...
    def instantiate_with_delete_effects(self, var_mapping, init_facts, fluent_facts, result):
        assert not result, "Condition not simplified"
        for part in self.parts:
            part.instantiate_with_delete_effects(var_mapping, init_facts, fluent_facts, result)
    def negate(self):
        return Disjunction([p.negate() for p in self.parts])
    def to_smt(self, timestamp):
        res = '(and '
        if len(self.parts) == 0:
            res += 'true'
        else:
            for var in self.parts:
                res += var.to_smt(timestamp)
        res += ')'
        return res
    def get_predicate(self, timestamp, declared_predicate, type='unknown'):
        for var in self.parts:
            declared_predicate = var.get_predicate(timestamp, declared_predicate, type)
        return declared_predicate
    def get_classical_problem_predicate(self):
        res = '(and '
        for var in self.parts:
            res += var.get_classical_problem_predicate()
        res += ') '
        return res
    def get_CPP_predicate(self):
        res = '(and '
        for var in self.parts:
            res += var.get_CPP_predicate()
        res += ') '
        return res

    def get_CPP_predicate_with_int(self, constant_precondition, contexts, int_index):
        res_inner = ''
        for var in self.parts:
            res = var.get_CPP_predicate_with_int(constant_precondition, contexts, int_index)
            if res is not None:
                res_inner += res
        if res_inner != '':
            return '(and ' + res_inner + ') '
        else:
            return None
    def get_classical_problem_predicate_with_timestamp(self, timestamp):
        res = '(and '
        for var in self.parts:
            res += var.get_classical_problem_predicate_with_timestamp(timestamp)
        res += ') '
        return res
    def get_separated_classical_problem_predicate(self, iteration):
        res = '(and '
        for var in self.parts:
            res += var.get_separated_classical_problem_predicate(iteration)
        res += ') '
        return res
    def get_merged_classical_problem_predicate(self, contexts):
        res = '(and '
        for var in self.parts:
            res += var.get_merged_classical_problem_predicate(contexts)
        res += ') '
        return res
    def get_CPP_merged_classical_problem_predicate(self, contexts):
        res = '(and '
        for var in self.parts:
            res += var.get_CPP_merged_classical_problem_predicate(contexts)
        res += ') '
        return res
    def get_merged_seperated_classical_problem_predicate(self, contexts, iteration, added_item):
        res = ''
        for var in self.parts:
            command = var.get_merged_seperated_classical_problem_predicate(contexts, iteration, added_item)
        ...
        if res != '':
            ...
            return None
    def get_superfd_predicate(self):
        res = '(and '
        for var in self.parts:
            res += var.get_superfd_predicate()
        res += ') '
        return res
...
class Disjunction(JunctorCondition):
    ...
    def get_merged_seperated_classical_problem_predicate(self, contexts, iteration, added_item):
        res = ''
        for var in self.parts:
            command = var.get_merged_seperated_classical_problem_predicate(contexts, iteration)
            if command is not None:
                res += command
        if res != '':
            return '(or ' + res + ') '
...
class Atom(Literal):
    ...
    def positive(self):
        return self
    def to_smt(self, timestamp):
        return self.predicate+''.join(self.args)+'-'+str(timestamp)+' '
    def to_string(self):
        return self.predicate + ''.join(self.args)
    def get_predicate(self, timestamp, declared_predicate, type='unknown'):
        declared_predicate[self.predicate+''.join(self.args)+'-'+str(timestamp)] = (type, self.predicate+' '+' '.join(self.args))
        return declared_predicate
    def get_classical_problem_predicate(self):
        return '( '+self.predicate + ' ' + ' '.join(self.args) + ' ?interpr) '
    def get_CPP_predicate(self):
        return '( ' + self.predicate + ' ' + ' '.join(self.args) + ' ?int) '
    def get_CPP_predicate_with_int(self, constant_precondition, contexts, int_index):
        if self in constant_precondition or self.negate() in constant_precondition:
            return self.get_superfd_predicate()
        if str(self.predicate) in contexts.all_predicate_name_in_contexts:
            return '( ' + self.predicate + ' ' + ' '.join(self.args) + ' int' +str(int_index) + ') '
    def get_classical_problem_predicate_with_timestamp(self, timestamp):
        return '('+self.predicate + ' ' + ' '.join(self.args) + ' int' + str(timestamp) + ') '
    def get_separated_classical_problem_predicate(self, iteration):
        return '('+self.predicate + ' ' + ' '.join(self.args) + ' int' + str(iteration) + ') '
    def get_merged_classical_problem_predicate(self, contexts):
        if self.predicate in contexts.all_predicate_name_in_contexts:
            return '( ' + self.predicate + ' ' + ' '.join(self.args) + ' ?interpr) '
        else:
            return '(' + self.predicate + ' ' + ' '.join(self.args) + ') '
    def get_CPP_merged_classical_problem_predicate(self, contexts):
    ...
    def get_merged_seperated_classical_problem_predicate(self, contexts, iteration, added_item):
        if self.predicate in contexts.all_predicate_name_in_contexts:
            command =  '( ' + self.predicate + ' ' + ' '.join(self.args) + ' int' + str(iteration) + ') '
        else:
            command =  '(' + self.predicate + ' ' + ' '.join(self.args) + ') '
        if command not in added_item:
            added_item.append(command)
            return command
        else:
            return None
    def get_superfd_predicate(self):
    ...
    def get_predicate_set(self):
        return {self}
    def get_formated_expression(self):
        return (self.predicate + ' ' + ' '.join(self.args)).strip()
    def get_pddl_expression(self):
        return ('(' + self.predicate + ' ' + ' '.join(self.args) + ')').strip()
...
class NegatedAtom(Literal):
    ...
    positive = negate
    def to_smt(self, timestamp):
        return '(not '+self.predicate + ''.join(self.args) + '-' + str(timestamp)+') '
    def to_string(self):
        return '(not ' + self.predicate + ''.join(self.args) + ')'
    def get_predicate(self, timestamp, declared_predicate, type='unknown'):
        declared_predicate[self.predicate + ''.join(self.args) + '-' + str(timestamp)] = (type, self.predicate+' '+' '.join(self.args))
        return declared_predicate
    def get_classical_problem_predicate(self):
        return '(not (' + self.predicate + ' ' + ' '.join(self.args) + ' ?interpr)) '
    def get_CPP_predicate(self):
        return '(not ( ' + self.predicate + ' ' + ' '.join(self.args) + ' ?int)) '
    def get_CPP_predicate_with_int(self, constant_precondition, contexts, int_index):
        if self in constant_precondition or self.negate() in constant_precondition:
            return self.get_superfd_predicate()
        if str(self.negate().predicate) in contexts.all_predicate_name_in_contexts:
            return '(not ( ' + self.predicate + ' ' + ' '.join(self.args) + ' int' + str(int_index) + ')) '
    def get_classical_problem_predicate_with_timestamp(self, timestamp):
        return '(not ( '+self.predicate + ' ' + ' '.join(self.args) + ' int' + str(timestamp) + ')) '
    def get_separated_classical_problem_predicate(self, iteration):
        return '(not ( '+self.predicate + ' ' + ' '.join(self.args) + ' int' + str(iteration) + ')) '
    def get_merged_classical_problem_predicate(self, contexts):
        if self.predicate in contexts.all_predicate_name_in_contexts:
            return '(not (' + self.predicate + ' ' + ' '.join(self.args) + ' ?interpr)) '
        else:
    ...
    def get_merged_seperated_classical_problem_predicate(self, contexts, iteration, added_item):
        if self.predicate in contexts.all_predicate_name_in_contexts:
            command =  '(not ( ' + self.predicate + ' ' + ' '.join(self.args) + ' int' + str(iteration) + ')) '
        else:
            command =  '(not (' + self.predicate + ' ' + ' '.join(self.args) + ')) '
        if command not in added_item:
            added_item.append(command)
            return command
        else:
    ...
    def get_predicate_set(self):
        return {self}
    def get_formated_expression(self):
        return '(not (' + (self.predicate + ' ' + ' '.join(self.args)).strip() + '))'
    def get_pddl_expression(self):
        return '(not (' + (self.predicate + ' ' + ' '.join(self.args)).strip() + '))'